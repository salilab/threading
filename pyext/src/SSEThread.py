...
class Loop():
    '''
    A Loop object stores a set of contiguous residues corresponding to an
    unmodeled length of sequence

    Loops also contain pointers to StructureElement object IDs immediately
    N-terminal and C-terminal to this loop.
    These IDs are key values for a StructureElement dictionary.

    If this loop starts at the N-terminus, then self.se_before_id = -1 and
    similarly is it is a C-terminal loop
    '''
    def __init__(self, chain_id="A", start=0, length=0, se_before_id=-1,
                 se_after_id=-1):
        self.start = start
    ...
    def generate_loop_id(self):
        # The loop ID is a unique identifier that contains all of the
        # information about this loop:
        # chain, start residue, length, SE before and SE after.
        return (self.chain_id + "_" + str(self.start) + "_" + str(self.length)
                + "_" + str(self.se_before_id) + "_" + str(self.se_after_id))

    def is_res_in_loop(self, residue):
        # Check if the given residue [as a tuple: (chain_id, resnum)]
        # is contained in this loop
        chain_id = residue[0]
        return resnum in self.get_loop_residues() and chain_id == self.chain_id


class SSEThread(IMP.ModelObject):
    def __init__(self, sequences={}, max_res_dist=4.0, se_clash_dist=4.0):
        '''
        @param sequences :: dictionary of sequences {chain_id:FASTA} or
               list of tuples [(chain_id, FASTA)]
        @param max_res_dist :: The maximum distance-per-residue (in angstroms)
               for computing loop lengths
        @param se_clash_dist :: Minimum distance that two SEs can be to each
               other (in angstroms)
        '''
        ...
        sel = IMP.atom.Selection(self.sequence_hierarchy)
        self.system_xyzs = [
            IMP.core.XYZ(p) for p in sel.get_selected_particles()]

    def _setup_SSE_system(self):
        self.root_hier = IMP.atom.Hierarchy.setup_particle(
            IMP.Particle(self.model))

        self.structure_chain = IMP.atom.Chain.setup_particle(
            IMP.Particle(self.model), "X")
        self.sequence_hierarchy = IMP.atom.Hierarchy.setup_particle(
            IMP.Particle(self.model))
    ...
    def build_sequence_chains(self, sequences):
        # Given a list of (chain_id, fasta sequence) tuples, build the
        # sequence chains

        for chain_id in sequences.keys():
            fasta = sequences[chain_id]
            new_seq_chain = IMP.atom.Chain.setup_particle(
                    IMP.Particle(self.model), str(chain_id))
            ...
            for i in range(len(fasta)):
                ...
                res = IMP.atom.Residue.setup_particle(
                    pr, alpha.get_residue_type_from_one_letter_code(fasta[i]),
                    sse_res_id)
                IMP.atom.Mass.setup_particle(
                    res.get_particle(),
                    IMP.atom.get_mass(res.get_residue_type()))
    ...
    def extract_structure_elements_from_smotif_pdbs(self, smotif_pdbs,
                                                    stride=None):
        '''
        ...
        for p in smotif_pdbs:
            ...
            # First, get CA atoms
            sel = IMP.atom.Selection(h, atom_type=IMP.atom.AT_CA)
            cas = sel.get_selected_particles()
            ...
            if stride is not None:
                ...
                self.structure_elements[se_ix] = \
                    self.setup_structure_element(coords0, "H", 0, 1, 0,
                                                 stride0)
                self.structure_elements[se_ix+1] = \
                    self.setup_structure_element(coords1, "H", 0, 1, 0,
                                                 stride1)
            else:
                self.structure_elements[se_ix] = \
                    self.setup_structure_element(coords0, "H", 0, 1, 0)
    ...
    def setup_structure_element(self, coordinates, sec_struct,
                                start_residue=0, polarity=1, offset=0,
                                chain_id="A", stride=None):
        ...
        for c in range(len(coordinates)):
            ...
            # radius doesn't really matter atm.
            xyz.set_radius(1.0)
            # Mass doesn't matter either
            IMP.atom.Mass.setup_particle(np, 1.0)
            se_hier.add_child(hp)

            if stride is not None:
                IMP.atom.SecondaryStructureResidue.setup_particle(
                    np, stride[c][0][0], stride[c][0][1], stride[c][0][2])

        se = IMP.threading.StructureElement.setup_particle(
            self.model, se_pi.get_index(), start_residue, polarity,
            len(coordinates), offset, chain_id)
        ...
        if sec_struct == "H":
            ...
            raise Exception(
                "Secondary structure designation must be H, S or C.",
                sec_struct, "was provided")
        return se
        ...
        for sid0 in range(len(sids)):
            ...
            for sid1 in range(sid0, len(sids)):
                ...
                if self.are_structure_elements_clashing(si0, si1):
                    ...
                    val = self.get_minimum_spanning_residues(
                        si0.get_coordinates()[-1], si1.get_coordinates()[0])
                    self.se_min_loop_table[sid0][sid1] = int(val)

                    val = self.get_minimum_spanning_residues(
                        si1.get_coordinates()[-1], si0.get_coordinates()[0])
    ...
    def get_minimum_spanning_residues(self, xyz1, xyz2):
        return math.ceil(
            IMP.algebra.get_distance(xyz1, xyz2) / self.max_res_dist)
    ...
    def get_start_res_list(self):
        ...
        for se in range(len(self.structure_elements.keys())):
            self.start_res_list.append(
                (self.structure_elements[se].get_chain(),
    ...
    def sort_seids(self, seids):
        ...
        seq_chains = [IMP.atom.Chain(c).get_id()
                      for c in self.sequence_hierarchy.get_children()]
    ...
    def get_loops(self, seids, set_as=True):
        """Given a set of StructureElement IDs, return a dictionary of
           Loop objects"""
        ...
        for chain_id in sorted_chain_ses_ids:
            ...
            # The first loop in a chain always starts at 1 and has no
            # SE N-terminal to it
            new_loop_start = 1
            new_se_before_id = -1
            ...
            for s in range(len(sorted_ses_ids)):
                ...
                new_loop_start = \
                    start_res + self.structure_elements[seid].get_length()
            # Now do the last loop
            length = \
                self.get_sequence_chain_lengths()[chain_id] - new_loop_start
    ...
    def get_available_start_residues(self, seid, same_loop=False):
        ...
        # If same loop, we want to only return residues that are within
        # the loop vacated by this SE
        if same_loop:
            before_loop, after_loop = self.get_loop_ids_bracketing_seid(seid)
            all_loop_resis = list(
                range(self.loops[before_loop].start,
                      self.loops[after_loop].start
                      + self.loops[after_loop].length))
            br = 0
            ar = 0
            if self.loops[before_loop].se_before_id != -1:
                lp = self.loops[before_loop]
                br = int(self.se_min_loop_table[lp.se_before_id][seid])
            if self.loops[after_loop].se_after_id != -1:
                lp = self.loops[after_loop]
                ar = int(self.se_min_loop_table[seid][lp.se_after_id])
            if len(all_loop_resis) - ar - se_len - br + 1 <= 0:
                print("No resis.  Weird!!")
                print("Same loop", seid, se.get_start_res(), se_len, "||",
                      before_loop, after_loop, "||", len(all_loop_resis), br,
                      ar, all_loop_resis[br:len(all_loop_resis)-ar-se_len+1])
                exit()
                return []
            else:
                for resnum in \
                        all_loop_resis[br:len(all_loop_resis)-ar-se_len+1]:
        ...
        for lk in loops.keys():
            ...
            if loop.length < 0:
                raise Exception("Loop length is less than zero", lk,
                                len(loops.keys()))
            ...
            if loop.se_before_id == -1:
                br = 0
            # Otherwise, see how many residues this SE needs to be from
            # the SE before
            else:
            ...
            # If, after subtracting the offsets, the loop is now too small,
            # continue without adding residues
    ...
    def get_loop_ids_bracketing_seid(self, seid):
        # From the loops dictionary, return the loop before and loop after
        # the given SEID
        # If there is no loop before or after (i.e., it's at a terminus),
        # return None for that element
    ...
    def compare_seids_and_loop_table(self):
        ...
        for lk in self.loops.keys():
            ...
            if lp.se_before_id != -1:
                se = self.structure_elements[lp.se_before_id]

                # Check that chains are the same
                if se.get_chain() != lp.chain_id:
                    print("Loop and SE have different chains!!", seid, lk)
                    good = False

                # Check that the loop starts one residue after the
                # N-terminal SE
                if se.get_resindex_list()[-1]+1 != lp.start:
                    print(se.get_resindex_list(), se.get_start_res(),
                          se.get_length())
                    print("--XX-- SEID", lp.se_before_id, se.get_start_res(),
                          se.get_resindex_list()[-1], ":: LOOP after",
                          lp.start, lp.start+lp.length, lp.get_loop_residues)
                    good = False

            # If not a C_terminal loop
            if lp.se_after_id != -1:
                se = self.structure_elements[lp.se_after_id]
                ...
                # Check that the C-terminal SE starts one residue after
                # the loop ends
                if se.get_start_res() != lp.start + lp.length:
                    print(se.get_resindex_list(), se.get_start_res(),
                          se.get_length())
                    print("--XX-- LOOP", lp.start, lp.start+lp.length,
                          lp.get_loop_residues(), "SEID After", lp.se_after_id,
                          se.get_start_res(), se.get_resindex_list()[-1],)
                    good = False

        # TODO:: Can't currently do this second check.  Need to get all
        # residues.
        '''
            ...
            for r in se.get_resindex_list():
                if r not in all_residues:
                    ...
                    for ix in self.get_built_structure_element_ids():
                        seix = self.structure_elements[ix]
                        if r in seix.get_resindex_list():
                            print("  ", ix, seix.get_resindex_list())
                    good=False
                else:
        ...
        for lk in self.loops:
            lp = self.loops[lk]
            if lp.se_before_id != -1:
                if lp.se_before_id in before_ids:
                    print("****", lp.se_before_id, "SEID is before two loops")
                    elmnt = self.structure_elements[lp.se_before_id]
                    print("SEID resis:", elmnt.get_resindex_list())
                    print("This loop:", lp.get_loop_residues())
                    for lkx in self.loops:
                        if self.loops[lkx].se_before_id == lp.se_before_id:
                            print("Other loop:",
                                  self.loops[lkx].get_loop_residues())
                    good = False
                else:
                    before_ids.append(lp.se_before_id)
            if lp.se_after_id != -1:
                if lp.se_after_id in after_ids:
                    print("****", lp.se_after_id, "SEID is after two loops")
                    elmnt = self.structure_elements[lp.se_after_id]
                    print("SEID resis:", elmnt.get_resindex_list())
                    print("This loop:", lp.get_loop_residues())
                    for lkx in self.loops:
                        if self.loops[lkx].se_after_id == lp.se_after_id:
                            print("Other loop:",
                                  self.loops[lkx].get_loop_residues())
                    good = False
                else:
                    after_ids.append(lp.se_after_id)
    ...
    def remove_se_from_loop_table(self, seid):
        ...
        if before_loop_id is not None:
            ...
            # And there is no SE before this loop now (since it starts
            # at N-terminus)
    ...
    def add_se_to_loop_table(self, seid, new_sr):
        # Given an SEID and a new start residue, modify the loop table
        # to account for this.
        ...
        # ensure no overlap:
        # TODO - reimplement for multi-chain. Add chain_id
        # to "get_built_residues".
        '''
        built = [IMP.atom.Residue(r).get_index()
                 for r in self.get_built_residues(chain_id=chain_id)]
        for r in range(new_sr,
                       new_sr+self.structure_elements[seid].get_length()):
            if r in built:
                print(built)
                print("What you doing!?!?!", r, "is already built",
                      self.structure_elements[seid].get_resindex_list())
                exit()
        '''
        ...
        if this_loop is None:
            ...
            raise Exception(
                "No loop found for SEID", seid, "in chain", chain_id,
                "starting at", resnum,
                [(self.loops[s].start,
                  self.loops[s].start+self.loops[s].length-1)
                 for s in self.loops.keys()])

        extra_string = ""
        # Mode is the number of new loops we make
        # mode = None
        # if the new start res is the start of this loop, then we only
        # create one new loop
        '''
        ...
        # If the new SE goes all the way to the end of this loop, then again,
        # only create one loop
        elif new_sr == this_loop.start+this_loop.length \
                - self.structure_elements[seid].get_length():
        ...
        if 0 == 0:
            ...
            if length < 0:
                print("HELP!", length, this_loop.start, this_loop.length, "|",
                      chain_id, start, resnum,
                      self.structure_elements[seid].get_length(), "|",
                      this_loop.generate_loop_id(), extra_string)
    ...
    def modify_loop_table(self, seid, new_sr):
        # Given a new start residue (as a (chain_id, resnum)), modify
        # the loop table for this seid
    ...
    def create_se_connectivity_restraints(self, structure_elements=None,
                                          function=None, n_sds=2):
        ...
        for i in range(len(structure_elements)-1):
            secr = IMP.threading.StructureElementConnectivityRestraint(
                self.model, function, self.secr_dummy_p0.get_index(),
                self.secr_dummy_p1.get_index(), n_sds)
    ...
    def update_SECR_restraints(self):
        ...
        for lk in self.loops.keys():
            if (self.loops[lk].se_before_id == -1
                    or self.loops[lk].se_after_id == -1):
                continue
            else:
                ...
                self.secrs[secr_id].assign_particles(
                    se0.get_particle_index(), se1.get_particle_index())
        ...
        for i in range(secr_id, n_secr):
            self.secrs[secr_id].assign_particles(
                self.secr_dummy_p0.get_index(), self.secr_dummy_p1.get_index())
    ...
    def add_SS_to_sequence(self, psipred, chain):
        ...
        for res in psipred.keys():
            sel = IMP.atom.Selection(
                self.sequence_hierarchy, residue_index=res, chain_id=chain)
            sys_part = sel.get_selected_particle_indexes()[0]
            IMP.atom.SecondaryStructureResidue.setup_particle(
                self.model, sys_part, psipred[res][0], psipred[res][1],
                psipred[res][2])
    ...
    def add_SS_to_ses(self, secstruct):
        for seid in secstruct.keys():
            ...
            for res in range(len(se_res)):
                ...
                IMP.atom.SecondaryStructureResidue.setup_particle(
                    self.model, se_res[res].get_particle_index(),
                    ss[0], ss[1], ss[2])
    ...
    def print_model(self):
        ...
        for lk in sorted_loop_keys:
            outstring += ":" + str(self.loops[lk].start) + "-loop-" + \
                    str(self.loops[lk].start + self.loops[lk].length - 1) + ":"
            if self.loops[lk].se_after_id != -1:
                se = self.structure_elements[self.loops[lk].se_after_id]
                outstring += \
                    ":" + str(se.get_start_res()) + "##STR##" + \
                    str(se.get_start_res() + se.get_length() - 1) + ":"
    ...
    def get_all_model_ss_propensities(self, unbuilt_as_coil=True):
        ...
        for seid in built_ses:
            ...
            for r in resids:
                ssr = IMP.atom.SecondaryStructureResidue(
                    self.model,
                    se_res[r-se.get_start_res()].get_particle_index())
                self.model_ss[r] = ssr.get_all_probabilities()
...
class CompletenessRestraint(IMP.Restraint):
    ...
    def __init__(self, root_hierarchy, slope=1.0):
        ...
        sel = IMP.atom.Selection(root_hierarchy, resolution=1)
        self.xyzs = [IMP.core.XYZ(p) for p in sel.get_selected_particles()]
        self.slope = slope
    ...
    def unprotected_evaluate(self, da):
        return ((len(self.xyzs)-self.get_number_of_built_residues())
                * self.slope)
...
class SecondaryStructureParsimonyRestraint(IMP.Restraint):
    def __init__(self, system, psipred, evaluate_unbuilt=True):
        ...
        # psipred_dictionary is formatted as:
        # { resnum:(frac_helix, frac_sheet, frac_coil), ... }
        self.system = system
        ...
        IMP.Restraint.__init__(self, self.system.model,
                               "SecondaryStructureParsimonyRestraint")

    def unprotected_evaluate(self, da):
        score = 0
        # First compute structured residues, as determined by built
        # structure elements
        model_ss = self.system.get_all_model_ss_propensities(
            unbuilt_as_coil=self.evaluate_unbuilt)
...
class SSEAdditionMover(IMP.threading.SSEThreadMover):
    def __init__(self, system, structure_elements):
        ...
        @param structure_elements :: A dictionary of structure elements
               with key - SEID
        '''
        se_pis = [structure_elements[seid].get_particle_index()
                  for seid in structure_elements.keys()]

        IMP.threading.SSEThreadMover.__init__(
            self, system.model, se_pis,
    ...
    def do_propose(self, new_start_res=None, seid=None):
        ...
        If no start residue (new_start_res) is given, choose one at random
        from the system
            ...
            for seid in rand_srs:
                ...
                if seid == rand_srs[-1]:
                    return IMP.core.MonteCarloMoverResult(
                        [se.get_particle_index()], 0.0)

        self.seid = seid  # Store the seid in a persistent variable
        se = self.structure_elements[self.seid]

        # Pick a random available start residue and change the SE start_res
        # to that
        if new_start_res is None:
            available_start_res = \
                self.system.get_available_start_residues(self.seid)
...
class SSEDeletionMover(IMP.threading.SSEThreadMover):
    ...
    def do_propose(self):
        ...
        # Don't need to use this since we don't change the chain
        self.old_chain = se.get_chain()
        se_pix = se.get_particle_index()

        # Change sequence coordinates to zero and set start_res_key of
        # SE to zero
        self.zero_coordinates(se_pix)
    ...
    def do_reject(self):
        ...
        # Reset the loops table, reset start_Res_key and transform the
        # coordinates back.
        self.system.add_se_to_loop_table(
            self.seid, (self.old_chain, self.old_start_res))
...
class SSEShiftMover(IMP.threading.SSEThreadMover):
    ...
    def do_propose(self):
        ...
        available_start_res = self.system.get_available_start_residues(
            self.seid, same_loop=self.same_loop)

        lasr = len(available_start_res)

        # Parse available start res by those only within N residues
        # in the same chain
        if self.max_disp is not None:
            available_start_res = [
                i for i in available_start_res
                if (sr[0] == self.old_chain and abs(i-sr[1]) <= self.max_disp)]

        if len(available_start_res) == 0:
            print("ShiftMover WARNING: No available residues returned for "
                  "this loop. Suspicious.", lasr)
            return IMP.core.MonteCarloMoverResult([se.get_particle_index()],
                                                  0.0)
...
class SEMover(IMP.threading.StructureElementMover):
    ...
    def __init__(self, system, seid, zero_pct=50):
        IMP.threading.StructureElementMover.__init__(
            self, system.model,
        ...
        if zero_pct > 0:
            if zero_pct < 1.0:
                ...
                raise Exception(
                    "zero_pct must be between zero and 100:", zero_pct)
        else:
    ...
    def do_propose(self):
        ...
        # Otherwise, try to move it to another residue OR set it to zero
            ...
            start_res = self.system.get_available_start_residues(
                self.seid, exclude_self=True)
            # simple hack to add zeroes
            start_res = start_res+[0]*int(len(start_res)*self.zero_pct/100.0)
...
def compare_threading_models(pdb_queries, pdb_reference,
                             close_ca_threshold=5.0,
                             exact_match_threshold=6.0):
    ...
    # Close:  Number of query CA atoms within 5 angstroms of any reference
    # CA atom.

    # Sequence:  Number of query CA atoms within 5 angstroms of exact
    # reference CA atom.
    ...
    coords = {}
    for rp in ref_parts:
        resid = IMP.atom.Residue(
            IMP.atom.Hierarchy(rp).get_parent()).get_index()
        coords[resid] = IMP.core.XYZ(rp).get_coordinates()

    for pdb in pdb_queries:
        h2 = IMP.atom.read_pdb(pdb, m, IMP.atom.CAlphaPDBSelector())

        query_parts = IMP.atom.Selection(
            h2, resolution=1).get_selected_particles()
        ...
        for pi in query_parts:
            resid = IMP.atom.Residue(
                IMP.atom.Hierarchy(pi).get_parent()).get_index()
            ...
            try:
                ...
                for ck in coords.keys():
                    if IMP.algebra.get_distance(
                            crds, coords[ck]) < close_ca_threshold:
                        n_ca_close += 1
                        break
            except:  # noqa: E722
                continue

        print(len(coords.keys()), len(query_coords.keys()), n_seq_close,
              n_ca_close)
...
class MonteCarlo():
    ...
    def get_proposal_move_set(self, num_moves):
        ...
        prop_additions = \
            (1.0 * (len(self.system.system_xyzs)
                    - len(self.system.get_built_residues()))
             / len(self.system.system_xyzs))*self.addition_pct
        prop_deletions = \
            1.0 * len(self.system.get_built_structure_element_ids()) \
            * self.deletion_pct / 100.0
        prop_shift = \
            1.0 * len(self.system.get_built_structure_element_ids()) \
            * self.shift_pct / 100.0

        moves = []
        for i in range(num_moves):
            rnum = numpy.random.rand() * (prop_additions + prop_deletions
                                          + prop_shift)
    ...
    for line in f.readlines():
        ...
        if fields[0] == "sequence":
            ...
            if not os.path.isfile(path):
                raise Exception("Localization table file", path,
                                "does not exist")
            ...
            config_dict["crosslink_dataset"].append(
                (path, float(fields[2]), float(fields[3]), float(fields[4])))

        else:
            # Everything else is a single parameter/pair
            # convert numbers to floats (will need to int all of the
            # integer parameters in the main code)
            try:
                val = float(fields[1])
            except:  # noqa: E722
                val = fields[1]
...
def is_config_dict_complete(config_dict):
    """Does this configuration dictionary have all of the required elements
       to run a simulation?"""
    ...
    if ("pdb_file" not in config_dict.keys()
            or "localization_table" not in config_dict.keys()):
        return False
...
default_configuration_file = \
    os.path.dirname(os.path.realpath(__file__)) + "/default_configuration.dat"
...
if __name__ == "__main__":
    ...
    parser.add_option("-b", "--benchmark", action="store_true",
                      ...
                      help="Run short benchmark for timing Only used with "
                           "'sampling' mode")
    parser.add_option("-o", "--output", dest="output_folder", type="string",
                      help="Directory to put/read output for this system.")
    parser.add_option("-t", "--truth", dest="truth", type="string",
                      default=None,
                      help="Reference PDB file for comparing "
                           "SSEThread results.")
    (options, args) = parser.parse_args(sys.argv[2:])

    config_dict = read_configuration_file(default_configuration_file,
                                          config_dict={})

    mode = sys.argv[1].lower()

    if mode == "setup":
        configuration = read_config_file(options.config_file,
                                         config_dict=config_dict)
        setup_ssethread(config_dict=configuration,
                        output_directory=options.output_folder)

    elif mode == "sampling":
        configuration = read_config_file(options.config_file,
                                         config_dict=config_dict)
        sample_ssethread(config_dict=configuration,
                         output_directory=options.output_folder,
                         benchmark=options.benchmark)

    elif mode == "analysis":
        analyze_ssethread(output_directory=options.output_folder,
                          truth=options.truth)
